...
impl Elemento {
    pub fn etiqueta(&self) -> &'static str {
        match self {
            Elemento::Fisico => "Physical",
            Elemento::Fuego => "Fire",
            Elemento::Hielo => "Ice",
            Elemento::Viento => "Wind",
            Elemento::Electrico => "Electric",
        }
    }
}
    ...
    let mensaje = match carta {
        CartaShuffle::Persona(persona) => {
            if stock.iter().any(|p| p.persona == persona.persona) {
                ...
                format!(
                    "You already own {}. It turns into {} EXP.",
                    persona.persona, exp
                )
            } else {
                stock.push(persona.clone());
                format!("You obtained {} (Lv.{})!", persona.persona, persona.nivel)
            }
        }
        CartaShuffle::Copa => {
            ...
            format!("Cup: you recover {} HP and 15 MP.", curacion)
        }
        CartaShuffle::Vara => {
            let exp = 25;
            jugador.experiencia += exp;
            format!("Rod: you gain {} EXP.", exp)
        }
        CartaShuffle::Vacia => String::from("Empty card... you got nothing."),
    };
    if subir_nivel(jugador) {
        format!("{} You leveled up to level {}!", mensaje, jugador.nivel)
    } else {
        mensaje
    }
}
